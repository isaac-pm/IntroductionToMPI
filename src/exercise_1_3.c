...
/* Helper macro for column-major indexing: A[i + j*ldA] */
    ...
    double const beta, double* const C, int const ldC)
{
    ...
    for (int j = 0; j < n; ++j) {
        for (int l = 0; l < k; ++l) {
            const double *Acol = A + IDX(0, l, ldA);
            double Blj = B[IDX(l, j, ldB)];
            for (int i = 0; i < m; ++i) {
                C[IDX(i, j, ldC)] += alpha * Acol[i] * Blj;
            }
        }
    }
}
    ...
    double const beta, double* const C, int const ldC)
{
    ...
    for (int j = 0; j < n; ++j) {
        for (int i = 0; i < m; ++i) {
            double sum = 0.0;
            for (int l = 0; l < k; ++l) {
                sum += A[IDX(i, l, ldA)] * B[IDX(l, j, ldB)];
            }
            C[IDX(i, j, ldC)] += alpha * sum;
        }
    }
}
