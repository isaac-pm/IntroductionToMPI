  ...
  double const* const A, double const* const B, double* const C)
{
    ...
    /* Calculate distribution parameters for B's columns */
    int local_n = n / size; 
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < size; ++i) {
            int cols = n / size + (i == size - 1 ? remainder : 0);
            sendcounts[i] = cols * k; // Elements per rank
            displs[i] = disp;
            disp += sendcounts[i];
        }
    }
    ...
    if (rank != 0) A_full = malloc(m * k * sizeof(double));
    else A_full = (double*)A; 

    MPI_Bcast(A_full, m * k, MPI_DOUBLE, 0, comm);

    /* Scatter B (columns) to all processes */
    double *B_local = malloc(k * local_n * sizeof(double));
    MPI_Scatterv(B, sendcounts, displs, MPI_DOUBLE, 
                 B_local, local_n * k, MPI_DOUBLE, 
                 0, comm);

    /* Local Computation: C_local = A * B_local */
    double *C_local = malloc(m * local_n * sizeof(double));
    
    multiply_matrices(m, k, local_n, A_full, B_local, C_local);

    /* Gather C results back to rank 0 */
    if (rank == 0) {
        // Recalculate counts for C (m rows * cols)
        int disp = 0;
        for (int i = 0; i < size; ++i) {
            int cols = n / size + (i == size - 1 ? remainder : 0);
            sendcounts[i] = cols * m; 
    ...
    MPI_Gatherv(C_local, local_n * m, MPI_DOUBLE,
